...
[tmux](http://tmux.sourceforge.net)는 사실 MadEye처럼 원격에서 집단 개발을 위해서 만들어진 도구는 아닙니다. 여기서 설명하려는건 원격에서 집단으로 개발할 때 사용하면 좋을 tmux의 한 기능을 소개하려고 합니다.

tmux에 대한 전반적인 설명은 아래 문서로 대신하고,

 - [TTY 멀티플랙서 tmux](http://blog.outsider.ne.kr/699)
 - [[cookbook] Tmux 입문자님들을 위해 #1 : 설치, 기본사용방법, session](http://nodeqa.com/nodejs_ref/99)

이제 tmux가 뭔지 대충 아셨나요? 이제 tumx를 이용하여 원격에서 집단으로 개발할때 유용한 기능을 소개해드리겠습니다.
...
※ 서버 소켓 파일이 생성되는 위치에 대한 것은 tmux man page에 보면 이렇게 설명되어 있습니다
> tmux stores the server socket in a directory under /tmp (or TMPDIR if set); the default socket is named default.

`/tmp`디렉터리에 `tmux-1000`이라는 디렉터리가 있는데, 이건 해당 명령을 내리는 사용자의 `uid`가 `1000`이기때문에 `tmux-1000` 디렉터리 밑에 `default`가 생성된겁니다.
...
이하 설명부터는 편의상 모든 사용자는 같은 계정(han)에 로그인한다고 가정합니다.

그리고 나서 다른사람이 똑같이 `$ tmux attach`했을때엔 최초에 누군가 session을 만들당시 생성된 `/tmp/tmux-1000/default`를 이용하여 같은 세션에 붙을 수 있는 겁니다.

여기까지만 보면 하나의 세션만 공유가 가능한 것 처럼 보이기도 하지요...?

지금부터 소개해드릴 옵션은 `-L`옵션과 `-S`옵션 입니다.
